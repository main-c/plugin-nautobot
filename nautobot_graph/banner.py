...
def banner(context, *args, **kwargs) -> Optional[Banner]:
    ...
    content += format_html(
        "<div>SAMPLE_VARIABLE is {}</div>",
        get_app_settings_or_config("nautobot_graph", "SAMPLE_VARIABLE"),
    )
    content += format_html(
        "<div>lowercase_example is {}</div>",
        get_app_settings_or_config("nautobot_graph", "lowercase_example"),
    )
