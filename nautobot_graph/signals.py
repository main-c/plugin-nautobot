"""Signal handlers for the example nautobot_graph."""
nautobot_graph_CUSTOM_FIELD_DEFAULT = "Default value"
nautobot_graph_CUSTOM_FIELD_NAME = (
    "nautobot_graph_auto_custom_field"  # Note underscores rather than dashes!
)
...
def nautobot_database_ready_callback(sender, *, apps, **kwargs):
    ...
    if hasattr(CustomField, "slug"):
        field, _ = CustomField.objects.update_or_create(
            slug=nautobot_graph_CUSTOM_FIELD_NAME,
            defaults={
                "type": CustomFieldTypeChoices.TYPE_TEXT,
                "label": "Example Plugin Automatically Added Custom Field",
                "default": nautobot_graph_CUSTOM_FIELD_DEFAULT,
            },
        )
    else:
        field, _ = CustomField.objects.update_or_create(
            key=nautobot_graph_CUSTOM_FIELD_NAME,
            defaults={
                "type": CustomFieldTypeChoices.TYPE_TEXT,
                "label": "Example Plugin Automatically Added Custom Field",
                "default": nautobot_graph_CUSTOM_FIELD_DEFAULT,
