...
class ExampleDryRunJob(Job):
    ...
    class Meta:
        ...
        description = (
            "Example job to remove serial number on all devices, supports dryrun mode."
        )
...
class ExampleJob(Job):
    ...
    template_name = "nautobot_graph/example_with_custom_template.html"
...
class ExampleLoggingJob(Job):
    ...
    def run(self, interval):
        ...
        for step in range(1, interval + 1):
            ...
            print(
                f"stderr logging for step {step}, task: {self.request.id}",
                file=sys.stderr,
            )
        ...
        self.logger.info(
            "Success", extra={"object": self.job_model, "grouping": "job_run_success"}
        )
...
class ExampleJobHookReceiver(JobHookReceiver):
    ...
    def receive_job_hook(self, change, action, changed_object):
        ...
        if "serial" in snapshots["differences"]["added"]:
            ...
            self.logger.info(
                "%s serial has been changed from %s to %s",
                changed_object,
                old_serial,
                new_serial,
            )
            ...
            if not self.validate_serial(new_serial):
                ...
                changed_object.save()
                self.logger.info(
                    "%s serial %s was not valid. Reverted to %s",
                    changed_object,
                    new_serial,
                    old_serial,
                )
    ...
    def receive_job_button(self, obj):
        ...
        if isinstance(obj, Location):
            if not user.has_perm("dcim.add_location"):
                self.logger.error(
                    "User '%s' does not have permission to add a Location.",
                    user,
                    extra={"object": obj},
                )
            else:
            ...
            if not user.has_perm("dcim.add_device"):
                self.logger.error(
                    "User '%s' does not have permission to add a Device.",
                    user,
                    extra={"object": obj},
                )
            else:
            ...
            self.logger.error(
                "Unable to run Job Button for type %s.",
                type(obj).__name__,
                extra={"object": obj},
            )
